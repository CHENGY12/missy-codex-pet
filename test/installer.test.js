...
test("installs the bundled v2 Missy package", async () => {
  ...
  assert.equal(result.displayName, "Missy Poop, Peek & Run (v2.3.0)");
});
...
test("rejects unknown Missy versions with the available choices", async () => {
  ...
  await assert.rejects(
    ...
    /available versions: 2\.0\.0, 2\.1\.1, 2\.1\.2, 2\.2\.0, 2\.2\.1, 2\.3\.0/
  );
});
...
test("installs the original and latest editions side by side", async () => {
  ...
  const latest = await addPet({ petId: "missy", version: "2.3.0", codexDirectory: root });
  ...
  assert.equal(latestManifest.displayName, "Missy Poop, Peek & Run (v2.3.0)");
});
