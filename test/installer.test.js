...
test("rejects unknown Missy versions with the available choices", async () => {
  ...
  await assert.rejects(
    ...
    /available versions: 2\.0\.0, 2\.1\.1, 2\.1\.2/
  );
});
...
test("installs the original and latest editions side by side", async () => {
  ...
  const latest = await addPet({ petId: "missy", version: "2.1.2", codexDirectory: root });
  ...
  assert.equal(latestManifest.displayName, "Missy Stretch & Meow (v2.1.2)");
});
