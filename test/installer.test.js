...
test("installs the bundled v2 Missy package", async () => {
  ...
  assert.equal(result.displayName, "Missy (v2.3.2)");
  assert.equal(
...
test("keeps the previous Missy v2.1.1 package available", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "missy-v2-1-1-"));
  const result = await addPet({
    petId: "missy",
    version: "2.1.1",
    codexDirectory: root
...
test("keeps the previous Missy v2.1.2 package available", async () => {
  ...
  assert.equal(result.version, "2.1.2");
  assert.equal(result.id, "missy");
  assert.equal(result.displayName, "Missy Stretch & Meow (v2.1.2)");
  assert.equal(result.changed, true);
});
...
test("keeps the previous Missy v2.2.1 stable-idle package available", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "missy-v2-2-1-"));
  const result = await addPet({
    petId: "missy",
    version: "2.2.1",
    codexDirectory: root
...
test("keeps the previous Missy v2.3.1 package available", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "missy-v2-3-1-"));
  const result = await addPet({
    petId: "missy",
    version: "2.3.1",
    codexDirectory: root
...
test("rejects unknown Missy versions with the available choices", async () => {
  ...
  await assert.rejects(
    ...
    /available versions: 2\.0\.0, 2\.1\.1, 2\.1\.2, 2\.2\.0, 2\.2\.1, 2\.3\.1, 2\.3\.2/
  );
});
...
test("installs the original and latest editions side by side", async () => {
  ...
  const latest = await addPet({ petId: "missy", version: "2.3.2", codexDirectory: root });
  ...
  assert.equal(latestManifest.displayName, "Missy (v2.3.2)");
});
