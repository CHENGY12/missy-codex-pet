#!/usr/bin/env python3
"""Build Missy v2.3.0 with native left-ear anatomy and poop-and-peek failure."""
...
LEFT_FRAMES_DIR = ROOT / "source/v2.3.0-running-left-frames"
NORMALIZED_LEFT_ROW = ROOT / "source/v2.3.0-running-left-row.png"
VERSION_DIR = ROOT / "versions/2.3.0/missy"
...
def write_manifest() -> None:
    manifest = {
        ...
        "description": (
            "Missy v2.3.0 combines the stable idle and stretch-and-meow actions with "
            "the cute poop-and-peek failed state, and repairs running-left with her "
            "anatomically correct asymmetric ear markings."
        ),
...
def write_reports(atlas: Image.Image, changed_rows: list[int]) -> None:
    ...
    patch_report = {
        ...
        "nativeRunningLeftFrames": "./source/v2.3.0-running-left-frames",
    ...
    (qa_dir / "v2.3.0-row-patch.json").write_text(
    ...
    review = {
        ...
        "runningLeft": {
            ...
            "frameCount": 8,
            "mirrorDerived": False,
            "nearEar": "Missy's anatomical left ear, dark black",
            "farEar": "Missy's anatomical right ear, pale pink/white with dark-orange base",
        },
        "failed": {
    ...
    (qa_dir / "v2.3.0-review.json").write_text(
    ...
    summary.update(
        {
            ...
            "running_left": "native asymmetric-ear repair; not mirrored",
            "validation": "./qa/atlas-validation.json",
            "review": "./qa/v2.3.0-review.json",
            "v2.3.0_row_patch": "./qa/v2.3.0-row-patch.json",
        }
    )
...
print "Missy Poop, Peek & Run (v2.3.0) was installed at ${pet_target}."
"""
    readme = """# Missy Poop, Peek & Run — v2.3.0
...
Run `npx --yes github:CHENGY12/missy-codex-pet add missy@2.3.0 --force` to preserve an existing Missy as a backup and install this version.
...
"""
    ...
    for output in (
        ROOT / "dist/missy-codex-pet-v2.3.0.zip",
        ...
        with zipfile.ZipFile(output, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
            for name, data in entries.items():
                ...
                info.date_time = (2026, 7, 20, 18, 0, 0)
...
def main() -> None:
    ...
    build_distribution()
    print(f"Built Missy v2.3.0: {VERSION_DIR / 'spritesheet.webp'}")
